...
import { MatTableDataSource, MatSort, MatSnackBar, PageEvent, Sort, MatSelect, MatDialog, MAT_DIALOG_DATA } from '@angular/material';
...
import { Funcionario } from 'src/app/funcionario/models/funcionario.model';
...
export class ListagemComponent implements OnInit {
  ...
  colunas: string[] = ['dataLancamento', 'tipoLancamento', 'localizacao', 'acao'];
  ...
  ordemPadrao() {
    this.ordem = 'dataLancamento';
    this.direcao = 'DESC';
  }

  exibirLancamentos() {

    if(isUndefined(this.matSelect.selected) ) {
  ...
  /**
   * Armazena um funcionarioId no sessioStorage do navegador.
   * Criar um cache para o funcionario em questão.
   */
  get funcId(): string {
  ...
  remover(lancamentoId: string) {
    this.lancamentoService.remover(lancamentoId)
      .subscribe(
        ...
        err => {
          let msg: string = "Erro ao remover lançamento. /n Tente em instantes.";
          if (err.status == 400) {
